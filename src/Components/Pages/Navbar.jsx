...
const Navbar = () => {
    ...
    return (
        <nav className={`fixed top-0 left-0 w-full z-50 border-b-[1px] border-[#f5efef32] transition-all duration-300 px-4 lg:px-2 md:px-8 ${isScrolled ? " bg-white" : "bg-transparent"
            }`}>
            ...
            <div className='max-w-7xl mx-auto flex  items-center justify-between py-5'>
                ...
                <div className='flex items-center gap-8'>
                    ...
                    <div className='flex relative md:left-32 lg:left-0 items-center gap-4 '>
                        {/* <IoIosSearch className='text-2xl text-[#F1D6C6] font-extrabold'></IoIosSearch> */}
                        <IoSearchSharp className={`text-2xl hidden md:block ${isScrolled ? "text-black" : "text-[#F1D6C6]"}`}></IoSearchSharp>
            ...
            {/* Mobile & Tablet Menu */}
            {menuOpen && (
                <div className="lg:hidden bg-white text-[#0D1216] font-semibold px-7 py-5 space-y-4">
                    <ul className="flex flex-col gap-4 text-[14px]">
                        {["Home", "About Us", "Shop", "Blog", "Contact Us"].map((item, i) => (
                            <li key={i} className="cursor-pointer border-b-1 py-1 border-b-orange-100">{item}</li>
                        ))}
                    </ul>
                </div>
            )}
